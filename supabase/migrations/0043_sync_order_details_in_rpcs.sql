-- ============================================================================
-- BOW — Migration 0043: ĐỒNG BỘ ĐẦY ĐỦ THÔNG TIN ĐƠN HÀNG (PLAN_ID, QUANTITY, IS_CTV_ORDER)
-- ============================================================================
...
begin
  ...
  -- 8. Tạo đơn hàng với đầy đủ thông tin
...
begin
  ...
  -- 3. Tính ưu đãi chào mừng đơn đầu nếu là đơn đầu và không phải CTV
  if v_user_role <> 'ctv' then
    select count(*) into v_valid_orders_count
    from public.orders
    where user_id = p_user_id
  ...
  if p_coupon_code is not null and trim(p_coupon_code) <> '' then
    ...
    if v_coupon_record.minimum_order_amount > 0 and v_original_total < v_coupon_record.minimum_order_amount then
      return jsonb_build_object('success', false, 'message', 'Đơn hàng chưa đạt giá trị tối thiểu để áp dụng mã.');
    end if;
    ...
    if v_coupon_record.first_order_only then
      ...
      if v_valid_orders_count > 0 then
        return jsonb_build_object('success', false, 'message', 'Mã giảm giá chỉ áp dụng cho đơn hàng đầu tiên.');
      end if;
    end if;
