...
export function fetchByCategory(type: ProductType): Promise<CatalogItem[]> {
  return withCache(`cat:${type}`, 60_000, async () => {
    try {
      ...
      console.warn('Supabase fetch failed for category:', type, e);
    }
    return [];
  });
}
...
export async function fetchAllProducts(): Promise<CatalogItem[]> {
  try {
    ...
    console.warn('Supabase fetch failed for all products:', e);
  }
  return [];
}
...
export async function fetchBySlug(slug: string): Promise<CatalogItem | null> {
  try {
    ...
    console.warn('Supabase fetch failed for slug:', slug, e);
  }
  return null;
}
...
export async function fetchFaqs(productId?: string): Promise<Faq[]> {
  try {
    ...
    if (productId && isValidUuid(productId)) {
      ...
      // Nếu productId không phải UUID hợp lệ (mock id), không query db để tránh lỗi 400 Invalid UUID
      return [];
    } else {
    ...
    console.warn('fetchFaqs warning:', err);
    return [];
  }
}
