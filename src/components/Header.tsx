...
export default function Header() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 w-full z-50 border-b transition-all duration-300 backdrop-blur-md ${scrolled
          ...
          <>
            ...
            <nav className="hidden lg:flex items-center gap-5 text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 ml-6 shrink-0">
              <NavLink
                to="/products"
                className={({ isActive }) =>
                  `flex items-center gap-1.5 relative py-1 transition-colors duration-200 whitespace-nowrap after:absolute after:bottom-[-4px] after:left-0 after:h-[2px] after:bg-[#00A3FF] after:transition-all after:duration-300 ${isActive
            ...
            <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
                      ...
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((notif) => (
                          <div
                            ...
                            className={`group flex items-start gap-3 rounded-xl p-2.5 transition cursor-pointer ${notif.is_read
              ...
              {/* User Profile / Login - Desktop Only (Mobile dùng Bottom Bar) */}
              {loading ? (
                <div className="hidden md:block h-8.5 w-8.5 animate-pulse rounded-full bg-slate-100 dark:bg-slate-800" />
              ) : session ? (
                <div className="relative user-menu-container hidden md:block">
                  <button
                    onClick={() => setShowUserMenu((v) => !v)}
                    className={`flex items-center gap-1.5 md:gap-2 rounded-full transition duration-200 focus:outline-none cursor-pointer ${isAdmin
                    ...
                    <div className="py-1.5 space-y-0.5">
                      ...
                      {isAdmin && (
                        <Link
                          to="/admin"
                          onClick={() => setShowUserMenu(false)}
                          className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left text-xs font-extrabold text-[#2563EB] dark:text-[#35A8FF] bg-blue-50/60 dark:bg-blue-950/40 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition mt-0.5"
                        >
                          <span className="text-sm">🛡️</span>
