...
export default function AdminTickets() {
  ...
  const ticketQuery = searchParams.get('ticket') || searchParams.get('id') || searchParams.get('q') || searchParams.get('search');

  // Sync URL search param ?ticket=BOW-1001 with selectedTicketId & searchQuery
  useEffect(() => {
    ...
    const found = tickets.find(
      (t) =>
        t.ticket_number.toLowerCase() === ticketQuery.toLowerCase() ||
        t.id === ticketQuery ||
        t.ticket_number.toLowerCase().includes(ticketQuery.toLowerCase())
    );
    if (found) {
  ...
  const handleOpenTicket = (t: AdminTicketRow) => {
    ...
    setSearchParams({ ticket: t.ticket_number });
  };
  ...
  const getStatusBadge = (status: AdminTicketRow['status']) => {
    switch (status) {
      case 'pending':
        return (
          <span className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full bg-rose-500/10 dark:bg-rose-500/15 border border-rose-500/20 px-3 py-1 text-xs font-bold text-rose-600 dark:text-rose-400">
      ...
      case 'processing':
        return (
          <span className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full bg-amber-500/10 dark:bg-amber-500/15 border border-amber-500/20 px-3 py-1 text-xs font-bold text-amber-600 dark:text-amber-400">
      ...
      case 'resolved':
        return (
          <span className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full bg-emerald-500/10 dark:bg-emerald-500/20 border border-emerald-500/30 px-3 py-1 text-xs font-bold text-emerald-600 dark:text-emerald-400">
      ...
      case 'closed':
        return (
          <span className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full bg-slate-500/10 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 px-3 py-1 text-xs font-bold text-slate-500 dark:text-slate-400">
  ...
  const getPriorityBadge = (priority: AdminTicketRow['priority']) => {
    switch (priority) {
      case 'urgent':
        return (
          <span className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg bg-red-500/15 border border-red-500/30 px-2.5 py-1 text-[10px] font-black uppercase text-red-600 dark:text-red-400 tracking-wide">
      ...
      case 'high':
        return (
          <span className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg bg-amber-500/15 border border-amber-500/30 px-2.5 py-1 text-[10px] font-black uppercase text-amber-600 dark:text-amber-400 tracking-wide">
      ...
      case 'normal':
        return (
          <span className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg bg-blue-500/15 border border-blue-500/30 px-2.5 py-1 text-[10px] font-black uppercase text-blue-600 dark:text-blue-400 tracking-wide">
      ...
      case 'low':
        return (
          <span className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg bg-slate-500/10 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 px-2.5 py-1 text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400 tracking-wide">
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tickets Table */}
      <div className="rounded-[24px] border border-[#E8F1FF] dark:border-[#1E2A4A]/50 bg-white dark:bg-[#131C32] p-5 shadow-xs overflow-hidden">
        {loading ? (
            ...
            <table className="w-full text-left text-xs font-semibold">
              <thead>
                <tr className="text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-100 dark:border-slate-800/50">
                  ...
                  <th className="py-3 px-3 whitespace-nowrap min-w-[110px]">Ưu tiên</th>
                  <th className="py-3 px-3 whitespace-nowrap min-w-[130px]">Trạng thái</th>
                  <th className="py-3 px-3 text-right whitespace-nowrap min-w-[110px]">Cập nhật cuối</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40 text-slate-700 dark:text-slate-300">
                {paginatedTickets.map((t) => (
                  ...
                  >
                    <td className="py-3.5 px-3 font-mono font-bold text-[#2563EB] dark:text-[#35A8FF] whitespace-nowrap">
                      {t.ticket_number}
                    </td>
                    <td className="py-3.5 px-3">
                      <span className="font-extrabold text-slate-900 dark:text-white block truncate max-w-[180px]">
                    ...
                    <td className="py-3.5 px-3">
                      <span className="font-extrabold text-slate-900 dark:text-white max-w-[200px] block truncate" title={t.subject}>
                        {t.subject}
                      </span>
                    </td>
                    <td className="py-3.5 px-3">
                      {t.orders ? (
                        <div>
                          <span className="font-bold text-slate-800 dark:text-slate-200 block truncate max-w-[160px]">{t.orders.product_name}</span>
                          <span className="font-mono text-[10px] text-slate-400">#{t.orders.payment_code}</span>
                        </div>
                      ) : (
