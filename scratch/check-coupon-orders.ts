...
async function checkCouponOrders() {
  const { data: couponOrders } = await adminDb
    .from('orders')
    .select('id, payment_code, product_name, price, original_price, discount_amount, coupon_code, status')
    .not('coupon_code', 'is', null);

  console.log('Orders with coupon:', couponOrders);
}

checkCouponOrders().catch(console.error);
