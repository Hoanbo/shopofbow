...
async function checkGrok() {
  const { data: grok } = await adminDb.from('products').select('*').eq('slug', 'grok-premium').single();
  console.log('Grok in DB:', grok);

  const { data: reviews } = await adminDb.from('product_reviews').select('*').eq('product_id', grok?.id);
  console.log('Grok reviews:', reviews);
}

checkGrok().catch(console.error);
